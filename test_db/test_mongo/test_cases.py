...
class TestCase(Protocol):
    ...
    def measure_exec_time(
            cls, db: Database, movie_id: str, user_id: str,
    ) -> float:
        pass
...
class TestGetUserFavorites:
    ...
    def measure_exec_time(db: Database, movie_id: str, user_id: str) -> float:
        ...
        exec_time_ms = (
            result.explain()['executionStats']['executionTimeMillis']
        )
        return exec_time_ms / 1000


class TestCountMovieLikes:
    @staticmethod
    def measure_exec_time(db: Database, movie_id: str, user_id: str) -> float:
        start = time.time()
        collection = db.estimations
        collection.count_documents(
            {'movie_id': movie_id, 'estimation': {'$gt': 7}}
        )
        return time.time() - start


class TestGetUserPostponed:
    @staticmethod
...
class TestGetAverageEstimation:
    @staticmethod
    def measure_exec_time(db: Database, movie_id: str, user_id: str):
        collection = db.estimations
        pipeline = [
            ...
            {
                '$group': {
                    '_id': '$movie_id', 'avg_estimation': {'$avg': '$estimation'}
                }
            },
        ]
        start = time.time()
